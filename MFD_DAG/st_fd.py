...
def read_input_standard(graphfile, weighttype):
    ...
    for i in range(0, len(graphList) - 1):
        ...
        finalGraphs.append({'vertices': list(in_neighbors.keys()), 'edges' : edges, 'out_neighbors': out_neighbors, 'in_neighbors': in_neighbors, 'source': source, 'sink': sink, 'max_flow': max_flow})

    return finalGraphs

def read_subpath_input(graphfile, weighttype):
    lines = open(graphfile,'r').readlines()
    ...
    #iterate thru start and end points
    for i in range(0, len(graphList) - 1, 2):
        num_nodes = 0
        edges = dict()
        out_neighbors = dict()
        in_neighbors = dict()
        ...
        while(True):
            ...
            if line[0] == 's':
                ...
                subpaths[j] = [(a,b) for a,b in more_itertools.pairwise(line[1:len(line) - 1])]
                subpaths_weights[j] = line[len(line) - 1]
                j = j + 1
            h = h + 1

        #return subpaths,subpaths_weights
        finalGraphs.append({'subpaths': subpaths, 'subpaths_weights': subpaths_weights, 'vertices': list(in_neighbors.keys()), 'edges' : edges, 'out_neighbors': out_neighbors, 'in_neighbors': in_neighbors, 'source': source, 'sink': sink, 'max_flow': max_flow})
...
def read_input_inexact(graphfile, weighttype):
    ...
    for i in range(0, len(graphList) - 1):
        ...
        finalGraphs.append({'vertices': list(in_neighbors.keys()), 'fup' : fup,'fdown': fdown, 'out_neighbors': out_neighbors, 'in_neighbors': in_neighbors, 'source': source, 'sink': sink, 'max_flow': max_flow})

    return finalGraphs

def write_reset(outputfilename):
    outputfile = open(outputfilename, 'w')
    outputfile.close()

def write_timePaths(outputfilename, time, paths):

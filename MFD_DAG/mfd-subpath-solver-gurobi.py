...
graphList = read_subpath_input(args.input, weighttype)
curr = 0

write_reset(args.output)

for graph in graphList:
    start = time.time()

    subpaths = graph ['subpaths']
    subpaths_weights = graph['subpaths_weights']
    edges = graph['edges']
    ...
    write_timePaths(args.output, (end-start), paths)
